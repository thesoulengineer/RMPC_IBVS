...
class IBVS_Controller():
    ...
    <ol>
        <li>Instantiate the controller with a control mode (listed above), interaction mode (listed above), and the number of points you will supply to the controller each iteration (must be greater than 0):<br><code>controller = IBVS_Controller(linear_velocities='xz', angular_velocities='', interaction_mode='curr', num_pts=2)</code></li>
    ...
    '''
    def __init__(self, linear_velocities: str, angular_velocities: str, interaction_mode: str, num_pts: int):
        assert linear_velocities is not None or angular_velocities is not None, f"You must include at least one output velocity."
        assert len(linear_velocities) > 0 or len(angular_velocities) > 0, f"You must include at least one output velocity."
        ...
        self.degrees_of_freedom = [False, False, False, False, False, False] # linear x y z, angular x y z

        for v in linear_velocities:
            if v == 'x':
                self.degrees_of_freedom[0] = True
            elif v == 'y':
                self.degrees_of_freedom[1] = True
            elif v == 'z':
                self.degrees_of_freedom[2] = True
            else:
                raise Exception(f"{v} is not a valid linear output velocity. Please refer to the class docstring to see the list of valid output velocities.")
            
        for v in angular_velocities:
            if v == 'x':
                self.degrees_of_freedom[3] = True
            elif v == 'y':
                self.degrees_of_freedom[4] = True
            elif v == 'z':
                self.degrees_of_freedom[5] = True
            else:
                raise Exception(f"{v} is not a valid angular output velocity. Please refer to the class docstring to see the list of valid output velocities.")
        
        self.num_degs = sum([1 for x in self.degrees_of_freedom if x == True])
    ...
    def _Le_rows(self, pts):
        '''
        Build the full interaction matrix L_e for the ACTIVE degrees of freedom
        from a list of points (each a tuple (x, y, z): normalized image coords +
        depth in meters). Returns a (2 * num_pts, num_degs) array.

        The per-point 2x6 interaction matrix follows Chaumette-Hutchinson:
            row_x = [ -1/z,    0,   x/z,   x*y,   -(1+x*x),   y ]   (vx vy vz wx wy wz)
            row_y = [    0, -1/z,   y/z, 1+y*y,      -x*y,   -x ]
        Only the columns whose DOF flag is True are kept, so the matrix matches
        the chosen control mode (e.g. '4xyzy' keeps vx, vy, vz, wy).
        '''
        rows = []
        dof = self.degrees_of_freedom            # [vx, vy, vz, wx, wy, wz]
        for (x, y, z) in pts:
            full = [
                (-1.0 / z,      0.0       ),      # vx
                ( 0.0,         -1.0 / z   ),      # vy
            ...
            rows.append([full[k][0] for k in range(6) if dof[k]])   # x-row
            rows.append([full[k][1] for k in range(6) if dof[k]])   # y-row
        return np.array(rows, dtype=float)       # (2*num_pts, num_degs)

    def calculate_interaction_matrix(self):
        '''
        Calculate the Moore-Penrose pseudoinverse of the error interaction matrix
        estimate, `self.L_e_est_pinv`, based on the current interaction mode.

            curr    : L_e_est = L_e(current points, current depth Z)
            desired : L_e_est = L_e(desired points, reference depth Z*)
            mean    : L_e_est = 0.5 * ( L_e + L_e_desired )   [needs Z and Z*]

        This realizes the mean-interaction IBVS law
            v = -lambda * ( 0.5 (L_s + L_s*) )^+ * e ,
        and works for any active DOF subset (2xz, 2zy, 4xyzy, full 6-DOF).
        '''
        if self.interaction_mode == 'curr':
            assert self.curr_pts is not None, "You must set the current points with set_current_points()."
            L = self._Le_rows(self.curr_pts)
            ...
            L = self._Le_rows(self.desired_pts)

        elif self.interaction_mode == 'mean':
            assert self.curr_pts is not None, "You must set the current points with set_current_points()."
            assert self.desired_pts is not None, "You must set the desired points with set_desired_points()."
            L = 0.5 * (self._Le_rows(self.curr_pts) + self._Le_rows(self.desired_pts))

        self.L_e_est_pinv = np.linalg.pinv(L)
...
if __name__ == '__main__':
    ...
    controller = IBVS_Controller(linear_velocities='xz', angular_velocities='', interaction_mode='curr', num_pts=2)
    ...
    controller = IBVS_Controller(linear_velocities='xz', angular_velocities='', interaction_mode='desired', num_pts=2)
    controller.set_lambda_matrix(lambdas=[5.0, 5.0])
    controller.set_desired_points(desired_pts=pts_2_center_near)
    controller.set_current_points(curr_pts=pts_2_center_far)
    controller.calculate_interaction_matrix()
    ...
    controller = IBVS_Controller(linear_velocities='xz', angular_velocities='', interaction_mode='mean', num_pts=2)
    controller.set_lambda_matrix(lambdas=[5.0, 5.0])
    controller.set_desired_points(desired_pts=pts_2_center_near)
    controller.set_current_points(curr_pts=pts_2_center_far)
    controller.calculate_interaction_matrix()
    ...
    controller = IBVS_Controller(linear_velocities='z', angular_velocities='y', interaction_mode='curr', num_pts=2)
    controller.set_lambda_matrix(lambdas=[5.0, 5.0])
    controller.set_desired_points(desired_pts=pts_2_center_near)
    controller.set_current_points(curr_pts=pts_2_center_far)
    controller.calculate_interaction_matrix()
    ...
    controller = IBVS_Controller(linear_velocities='z', angular_velocities='y', interaction_mode='desired', num_pts=2)
    controller.set_lambda_matrix(lambdas=[5.0, 5.0])
    controller.set_desired_points(desired_pts=pts_2_center_near)
    controller.set_current_points(curr_pts=pts_2_center_far)
    controller.calculate_interaction_matrix()
    ...
    controller = IBVS_Controller(linear_velocities='z', angular_velocities='y', interaction_mode='mean', num_pts=2)
    controller.set_lambda_matrix(lambdas=[5.0, 5.0])
    controller.set_desired_points(desired_pts=pts_2_center_near)
    controller.set_current_points(curr_pts=pts_2_center_far)
    controller.calculate_interaction_matrix()
    ...
    controller = IBVS_Controller(linear_velocities='xyz', angular_velocities='y', interaction_mode='curr', num_pts=4)
    ...
    controller = IBVS_Controller(linear_velocities='xyz', angular_velocities='y', interaction_mode='desired', num_pts=4)
    controller.set_lambda_matrix(lambdas=[5.0, 5.0, 5.0, 5.0])
    controller.set_desired_points(desired_pts=pts_4_center_near)
    controller.set_current_points(curr_pts=pts_4_center_far)
    controller.calculate_interaction_matrix()
    ...
    controller = IBVS_Controller(linear_velocities='xyz', angular_velocities='y', interaction_mode='mean', num_pts=4)
    controller.set_lambda_matrix(lambdas=[5.0, 5.0, 5.0, 5.0])
    controller.set_desired_points(desired_pts=pts_4_center_near)
    controller.set_current_points(curr_pts=pts_4_center_far)
    controller.calculate_interaction_matrix()
